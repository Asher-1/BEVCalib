#!/bin/bash
# V54d 泛化评估 + 双 ckpt 门控 gdiag
#
# 用法:
#   bash run_v54d_eval.sh --precheck-only
#   bash run_v54d_eval.sh --gate-eval
#   nohup bash run_v54d_eval.sh --gate-eval > logs/evaluations/generalization_v54d/nohup.log 2>&1 &
...
EVAL_CONFIG="configs/eval_generalization_v54d.yaml"
LOG_DIR="logs/evaluations/generalization_v54d"
GATE_LABEL="v54d-router-fix-gate"
mkdir -p "$LOG_DIR"

echo "========================================"
echo "V54d Generalization Evaluation"
...
if [ "$MODE" = "precheck" ]; then
    python - <<'PY'
import yaml, os
cfg = yaml.safe_load(open("configs/eval_generalization_v54d.yaml"))
