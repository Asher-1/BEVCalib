...
#
# 用法:
#   bash run_v54_eval.sh
#   bash run_v54_eval.sh --gate-eval
#   bash run_v54_eval.sh --precheck-only
#   bash run_v54_eval.sh --report-only
...
EVAL_CONFIG="configs/eval_generalization_v54.yaml"
LOG_DIR="logs/evaluations/generalization_v54"
GATE_LABEL="v54a-lsp-gate"
mkdir -p "$LOG_DIR"

echo "========================================"
echo "V54 Generalization Evaluation"
...
_run_gate_eval() {
    echo ""
    echo "--- V54a deploy gate eval (dual MEDW + val Recovery gdiag) ---"
    if python scripts/evaluate_deploy_gate.py \
        ...
        echo "[WARN] Gate eval failed — 检查 ckpt 是否存在"
    fi
}
...
if [ "$MODE" = "precheck" ]; then
    python - <<'PY'
import yaml, os
cfg = yaml.safe_load(open("configs/eval_generalization_v54.yaml"))
