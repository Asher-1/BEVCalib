#!/bin/bash
# V53f 泛化评估 + 双 ckpt 门控 gdiag
#
# 用法:
#   bash run_v53f_eval.sh --precheck-only
#   bash run_v53f_eval.sh --gate-eval
#   nohup bash run_v53f_eval.sh --gate-eval > logs/evaluations/generalization_v53f/nohup.log 2>&1 &
...
EVAL_CONFIG="configs/eval_generalization_v53f.yaml"
LOG_DIR="logs/evaluations/generalization_v53f"
GATE_LABEL="v53f-gin-jacg-gate"
mkdir -p "$LOG_DIR"

echo "========================================"
echo "V53f Generalization Evaluation"
...
if [ "$MODE" = "precheck" ]; then
    python - <<'PY'
import yaml, os
cfg = yaml.safe_load(open("configs/eval_generalization_v53f.yaml"))
