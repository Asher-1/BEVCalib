...
CKPT="${2:-logs/all_training_data/model_small_5deg_v78_dinov2_query_rpy02_recovery_guard_smoke/all_training_data_scratch/checkpoint/ckpt_best_medw.pth}"
LABEL="${LABEL:-v78_dinov2_query_rpy02_recovery_guard}"

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PY="${PY:-/opt/conda/envs/bevcalib310/bin/python}"
GPU="${GPU:-0}"
DATA="${DATA:-/mnt/drtraining/user/dahailu/data/bevcalib/test_data_v2}"
OUT_ROOT="${OUT_ROOT:-${ROOT}/logs/evaluations/v78_dinov2_query_rpy02/${LABEL}}"
...
CUDA_VISIBLE_DEVICES="${GPU}" "${PY}" "${ROOT}/tools/analysis/evaluate_p0_refinement.py" \
  ...
  --adaptive_seq_residual_deg "${ADAPTIVE_SEQ_RESIDUAL_DEG:-0.30}" \
  --adaptive_seq_residual_lo_deg "${ADAPTIVE_SEQ_RESIDUAL_LO_DEG:-0.18}" \
  --adaptive_seq_rpy_std_deg "${ADAPTIVE_SEQ_RPY_STD_DEG:-0.16}" \
  --rpy_threshold_deg "${RPY_THRESHOLD_DEG}" \
  "${FIXED[@]}" \
  "${EXTRA[@]}"

echo "V78 RPY<${RPY_THRESHOLD_DEG} eval output: ${OUT_DIR}"
